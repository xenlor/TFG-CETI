...
suricata_path = "../csv/logs_suricata_sensor_temperatura_convertido.csv"
ton_iot_path = "../csv/Train_Test_IoT_Thermostat.csv"
output_path = "../csv/dataset_combinado_temperatura.csv"
...
# 🟠 Eliminar las columnas innecesarias si las tuviera (ej. thermostat_status)
if "thermostat_status" in df_ton.columns:
    df_ton = df_ton.drop(columns=["thermostat_status"])
...
df_ton = df_ton[["date", "time", "temperature", "label", "type"]]
df_suricata = df_suricata[["date", "time", "temperature", "label", "type"]]
...
# 🟥 Eliminar duplicados (por date, time y temperature)
df_combinado = df_combinado.drop_duplicates(subset=["date", "time", "temperature"]).copy()
