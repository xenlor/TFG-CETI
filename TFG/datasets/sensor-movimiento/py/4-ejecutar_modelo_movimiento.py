...
def preprocesar_evento(evento: dict) -> pd.DataFrame | None:
    global ultimo_timestamp, ultimo_status

    mqtt_pub = evento.get("mqtt",{}).get("publish")
    if not mqtt_pub or mqtt_pub.get("topic")!="iot/sensor/movimiento":
        return None

    msg = mqtt_pub.get("message","").upper()
    if msg not in ("ON","OFF"):
        print(f"⚠️ Payload inválido MOVIMIENTO: '{msg}' → ataque")
        enviar_alerta_telegram(f"🚨 Invalid payload MOVIMIENTO: '{msg}' (IP {evento.get('src_ip','-')})")
    ...
    ultimo_timestamp, ultimo_status = ts, status
...
with open(eve_log,"r") as f:
    ...
    while True:
        ...
        val      = int(X.at[0,"motion_status"])
        dt_act   = X.at[0,"delta_time"]
        icono    = "🚨 ATAQUE" if pred==1 else "✅ Normal"

        print(f"{icono} → Motion: {val}, Δt: {dt_act:.3f}s (IP {src_ip})")
        enviar_alerta_elastic("movimiento",pred,val,dt_act,src_ip)

        if pred==1:
            contador_ataques+=1
            if contador_ataques>=UMBRAL:
                enviar_alerta_telegram(f"🚨 ¡{UMBRAL} ataques seguidos MOVIMIENTO! IP: {src_ip}")
                contador_ataques=0
        else:
            contador_ataques=0
