...
def send_block_prompt(ip: str):
    ...
    payload = {
      "chat_id": CHAT_ID,
      "text": f"🚨 Se han detectado *{UMBRAL} ataques consecutivos* desde la IP *{ip}* en el sensor HUMEDAD.\n¿Deseas bloquearla?",
      "parse_mode": "Markdown",
      "reply_markup": kb
    }
...
def preprocesar_evento(evt: dict):
    ...
    if ultimo_ts is None:
        ...
        if dt < 0.01 and val == ultimo_valor:
            return None
...
with open(EVE_LOG, "r") as f:
    ...
    while True:
        ...
        # indexar SIEMPRE
        indexar_elastic("humedad", pred, val, dt, ip)

        # prompt de bloqueo tras UMBRAL ataques
        if pred == 1:
            cont_ataques += 1
