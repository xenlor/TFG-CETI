...
def send_block_prompt(ip: str):
    ...
    payload = {
      "chat_id": CHAT_ID,
      "text": f"🚨 Se han detectado *{UMBRAL} ataques consecutivos* desde la IP *{ip}* en el sensor TEMPERATURA.\n¿Deseas bloquearla?",
      "parse_mode": "Markdown",
      "reply_markup": kb
    }
...
# ——————————————————————————————
def indexar_elastic(sensor: str, pred: int, valor: float, delta_time: float, src_ip: str):
    doc = {
        "timestamp": datetime.now().astimezone().isoformat(),
        "sensor": sensor,
        "resultado": "ataque" if pred == 1 else "normal",
        "valor": -1 if (isinstance(valor, float) and math.isnan(valor)) else valor,
...
def preprocesar_evento(evt: dict):
    ...
    if not pub or pub.get("topic")!="iot/sensor/temperatura":
        return None
    ...
    ultimo_ts = ts
    ultimo_val= val
    return pd.DataFrame([[val, dt]], columns=["temperature","delta_time"])
...
with open(EVE_LOG, "r") as f:
    ...
    while True:
        ...
        temp_act = df_evt.at[0,"temperature"]
        dt_act   = df_evt.at[0,"delta_time"]
        ip       = evt.get("src_ip","-")
        icon     = "🚨 ATAQUE" if pred == 1 else "✅ Normal"
        print(f"{icon} → Temperatura={temp_act:.2f}, Δt={dt_act:.3f}s (IP {ip})")

        indexar_elastic("temperatura", pred, temp_act, dt_act, ip)
